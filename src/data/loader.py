...
from .dataset import StreamDataset, Dataset, ParallelDataset, StreamMixDataset
from .dictionary import BOS_WORD, EOS_WORD, PAD_WORD, UNK_WORD, MASK_WORD
import pickle 
import pandas as pd
import re
import string
# load English - load Malaya
en_words = pd.read_csv('/home/nanda/XLM_modif/data/dict/en_words.txt', names=['text'])
en_words = en_words['text'].to_list()
ms_words = pd.read_excel("/home/nanda/XLM_modif/data/dict/malay.xlsx", names=['text'])
ms_words = ms_words['text'].to_list()

en_words = {k:1 for k in set(en_words)}
ms_words = {k:1 for k in set(ms_words)}
...
def process_mix_binarized(data, params, path='test_lang_embed'):
    """
    Process a binarized dataset and log main statistics.
    """
    dico = data['dico']
    ...
    # add lang_embedding
    data['languages'] = get_language_embedding(data['sentences'], dico)
    logger.info('================= Nanda Kepo : Languge Embedding')
    with open(path+'_LI.p', 'wb') as f:
        pickle.dump(data['languages'], f)
    return data

def check_dict(word, unfinish, count_token):
    tags = []
    tag = ''
    if word in ms_words:
        tag= 1 #ms
    elif re.match(r'[\u4e00-\u9fff]+', word) != None:
        tag= 2 #zh
    elif word in en_words:
        tag= 3 #en
    elif word in string.punctuation or re.match('[0-9]', word) != None:
        tag= 4 #other
    else :
        tag= 1
        
    if unfinish:
        tags = [tag] * count_token
    else:
        tags = tag
        
    return tags

def get_language_embedding(sentences, dico):
    tokens = []
    for idx in sentences:
        tokens.append(dico.id2word[idx])
    tmp_token = ''
    lang_tags = []
    unfinish = False
    idx = 0
    for i,token in enumerate(tokens):
        if i < len(tokens)-1:
            if re.match(r'$\@\@', token):
                unfinish = True
                t = token.replace('@@', '') 
                tmp_token = tmp_token + t
                idx = idx + 1
            else:
                if tmp_token != '' and re.match(r'$\@\@', token[i-1])!=None and token in string.punctuation:
                    unfinish = True
                    t = token.replace('@@', '') 
                    tmp_token = tmp_token + t
                    idx = idx + 1
                    lang_tags = lang_tags + check_dict(tmp_token, unfinish, idx)
                    tmp_token = ''
                    idx = 0
                elif tmp_token != '' and word in string.punctuation:
                    lang_tags = lang_tags + check_dict(tmp_token, unfinish, idx)
                    unfinish = False
                    lang_tags.append(4)
                else:
                    unfinish = False
                    lang_tags.append(check_dict(token, unfinish, 0))
        else:
            unfinish = False
            lang_tags.append(check_dict(token, unfinish, 0))

    return np.array(lang_tags)
...
def load_mix_binarized(path, params):
    """
    Load a binarized dataset.
    """
    assert path.endswith('.pth')
    ...
    logger.info("Loading data from %s ..." % path)
    data = torch.load(path)
    data = process_mix_binarized(data, params, path)
    return data
    ...
    for lang in params.mono_dataset.keys():
        ...
        for splt in ['train', 'valid', 'test']:
            ...
            if lang == 'mix':
                ...
                mono_data = load_mix_binarized(params.mono_dataset[lang][splt], params)
                set_dico_parameters(params, data, mono_data['dico'])

                data['mono_stream'][lang][splt] = StreamMixDataset(mono_data['sentences'], mono_data['positions'], mono_data['languages'], bs, params)
            else :
                # load data / update dictionary parameters / update data
...
def load_mix_data(params, data):
    ...
    for lang in params.mono_dataset.keys():
        ...
        logger.info('============ mixed data (%s)' % lang)

        assert lang in params.langs and lang not in data['mix']
        data['mix'][lang] = {}
        data['mix_stream'][lang] = {}
        ...
        for splt in ['train', 'valid', 'test']:
            ...
            # load data / update dictionary parameters / update data
            mix_data = load_mix_binarized(params.mix_dataset[lang][splt], params)
            set_dico_parameters(params, data, mix_data['dico'])

            # create stream dataset
            bs = params.batch_size if splt == 'train' else 1
            data['mix_stream'][lang][splt] = StreamMixDataset(mix_data['sentences'], mix_data['positions'], mix_data['languages'], bs, params)

            # if there are several processes on the same machine, we can split the dataset
            if splt == 'train' and params.split_data and 1 < params.n_gpu_per_node <= data['mix_stream'][lang][splt].n_batches:
                n_batches = data['mix_stream'][lang][splt].n_batches // params.n_gpu_per_node
                a = n_batches * params.local_rank
                b = n_batches * params.local_rank + n_batches
                data['mix_stream'][lang][splt].select_data(a, b)
...
def check_data_params(params):
    ...
    if 'mix' in params.langs:
        params.id2lang = {0:'mix', 1:'ms', 2:'zh', 3:'en', 4:'other'}
        params.lang2id = {'mix':0, 'ms':1, 'zh':2, 'en':3, 'other':4}
        params.n_langs = 5
    else:
    ...
    # MMLM / TLM steps
    mmlm_steps = [s.split('-') for s in params.mmlm_steps.split(',') if len(s) > 0]
    params.mmlm_steps = [(s[0], None) if len(s) == 1 else tuple(s) for s in mlm_steps]
    assert len(params.mmlm_steps) == len(set(params.mmlm_steps))
    ...
    # check mixlingual datasets
    required_mix = set(['mix'])
    params.mix_dataset = {
        lang: {
            splt: os.path.join(params.data_path, '%s.%s.pth' % (splt, lang))
            for splt in ['train', 'valid', 'test']
        } for lang in params.langs if lang in required_mix
    }
    for paths in params.mix_dataset.values():
        for p in paths.values():
            if not os.path.isfile(p):
                logger.error(f"{p} not found")
    assert all([all([os.path.isfile(p) for p in paths.values()]) for paths in params.mix_dataset.values()])
...
def load_data(params):
    logger.info('============ Params in load_data()')
    logger.info(params)
    """
    Load monolingual data.
    ...
    """
    data = {}
    logger.info("====================== nanda log: data befor load mono data")
    logger.info(data)

    # monolingual datasets
    load_mono_data(params, data)
    logger.info("====================== nanda log: data rafter load mono data")
    logger.info(data)
...
def load_data_mix(params):
    logger.info('============ isi params adalah :')
    logger.info(params)
    """
    Load monolingual data.
    The returned dictionary contains:
    ...
    # mixed datasets
    load_mix_data(params, data)

    # monolingual data summary
    logger.info('============ Data summary')
    for lang, v in data['mix_stream'].items():
        for data_set in v.keys():
            logger.info('{: <18} - {: >5} - {: >12}:{: >10}'.format('Monolingual data', data_set, lang, len(v[data_set])))
