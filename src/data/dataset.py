...
class StreamMixDataset(object):

    def __init__(self, sent, pos, lang, bs, params):
        """
        Prepare batches for data iterator.
        """
        bptt = params.bptt
        ...
        buffer = np.zeros(t_size, dtype=lang.dtype) + 3 # </s> inclued into english word
        buffer[t_size - n_tokens:] = lang
        buffer = buffer.reshape((bs, n_batches * bptt)).T
        self.data_lang = np.zeros((n_batches * bptt + 1, bs), dtype=lang.dtype) + 3 # </s> inclued into english word
        self.data_lang[1:] = buffer 
    ...
    def get_iterator(self, shuffle, subsample=1):
        ...
        for i in indexes:
            a = self.bptt * i
            b = self.bptt * (i + 1)
            yield torch.from_numpy(self.data[a:b].astype(np.int64)), torch.from_numpy(self.data_lang[a:b].astype(np.int64)), self.lengths
