...
class Trainer(object):
    ...
    def generate_batch(self, lang1, lang2, name):
        ...
        if lang1 == 'mix':
            x, langs, lengths = self.get_batch(name, lang1, stream=True)
            positions = None
        elif lang2 is None:
    ...
    def get_lang_embedding(self, x):
        return x

    def generate_batch_mix(self, lang1, lang2, name):
        """
        Prepare a batch (for causal or non-causal mode).
        """
        params = self.params
    ...
    def end_epoch(self, scores):
        ...
        if self.stopping_criterion is not None and (self.params.is_master or not self.stopping_criterion[0].endswith('_mt_bleu')):
            ...
            logger.info("============== nanda logger matric and biggest")
            logger.info(metric)
            logger.info(biggest)
    ...
    def clm_step(self, lang1, lang2, lambda_coeff):
        ...
        CLM objective.
        """
        logger.info('==== nanda check clm_step')
    ...
    def mlm_step(self, lang1, lang2, lambda_coeff):
        ...
        # forward / loss
        # gimana caranya biar yg masuk ke model ini nilai x sama lang nya sama
    ...
    def mix_lm_step(self, lang1, lang2, lambda_coeff):
        """
        Masked word prediction step.
        MLM objective is lang2 is None, TLM objective otherwise.
        """
        ...
        id2lang = {0: 'en', 1: 'ms', 2: 'zh'}
        lang2id = {'en': 0, 'ms' : 1, 'zh': 2}
        setattr(self.params, 'id2lang', id2lang)
        setattr(self.params, 'lang2id', lang2id)
        params = self.params 
        self.params = params
        name = 'model' if params.encoder_only else 'encoder'
        model = getattr(self, name)
        ...
        # generate batch / select words to predict
        x, lengths, positions, langs, _ = self.generate_batch_mix(lang1, lang2, 'pred')
        x, lengths, positions, langs, _ = self.round_batch(x, lengths, positions, langs)
        x, y, pred_mask = self.mask_out(x, lengths)
    ...
    def pc_step(self, lang1, lang2, lambda_coeff):
        logger.info('==== nanda check pc_step')
        """
...
class EncDecTrainer(Trainer):
    ...
    def mt_step(self, lang1, lang2, lambda_coeff):
        logger.info('==== nanda check mt_step')
        """
    ...
    def bt_step(self, lang1, lang2, lang3, lambda_coeff):
        logger.info('==== nanda check bt_step')
        """
