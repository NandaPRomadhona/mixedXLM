...
class SENTIFIN:
    ...
    def eval(self, splt):
        ...
        for batch in self.get_iterator(splt, 'mix'):
            ...
            # prediction
            vector = self.embedder.get_embeddings(x, lengths, positions=None, langs=None)
            output = self.proj(vector)
            p = output.data.max(1)[1]
            idxs.append(idx)
            # prob.append(output.cpu().numpy())
            pred.append(p.cpu().numpy())
            if splt == 'test':
                vec.append(vector.cpu().detach().numpy())
        ...
        if splt == 'test':
            logger.info('====== saved sentence representation')
            save_data = {}
            save_data['p'] = vec
            save_data['y_p'] = gold
            with open(params.data_path+'/p_'+ splt +'.p', 'wb') as f:
